...
convict.dat %>% 
  ...
  pairs.panels()
```
A quick glance at the chart shows that relatively stronger relationships exist between number of convictions and whether the child was difficult to discipline in school, the child's score on the progressive matrices and verbal comprehension (so, IQ), and familial criminal record variables. Of note is the moderate correlation between the two IQ variables, so perhaps we need only include one in the model. There also seems to be a relationship between parental attitudes and conduct disorder,which is interesting. Perhaps we ought to investigate this further. Aside from these possible instances of multicollinearity, the predictors appear to be relatively unrelated, so it is unlikely that we will see "overlapping" explained variance in the model. 

```{r}
convict.dat$v178 <- as.factor(convict.dat$v178)
convict.dat$v54 <- as.factor(as.numeric((convict.dat$v54)))

ggplot(data = convict.dat, mapping = aes(x = v178, y = v28)) + geom_point(mapping = aes(color = v54)) + geom_smooth(mapping = aes(color = v54))
```
